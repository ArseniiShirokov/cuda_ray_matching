...
Image Render(Scene** device_scene, const CameraOptions& camera_options, const RenderOptions& render_options) {
    ...
    checkCudaErrors(cudaEventElapsedTime(&elapsed_all, start_all, stop_all));

    checkCudaErrors(cudaEventDestroy(start_all));
    checkCudaErrors(cudaEventDestroy(stop_all));
